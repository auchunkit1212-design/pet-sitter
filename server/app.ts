...
export function createApp() {
  ...
  app.get('/api/availability', async (c) => {
    ...
    const booked = new Set(await getBookedSlots(date))
    const now = new Date()
  ...
  app.post('/api/create-checkout-session', async (c) => {
    ...
    if (!stripe) {
      ...
      return c.json({
        url: `${siteUrl}/?booking=success&session_id=${booking.stripeSessionId}#booking`,
        demo: true,
      })
    }
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      success_url: `${siteUrl}/?booking=success&session_id={CHECKOUT_SESSION_ID}#booking`,
      cancel_url: `${siteUrl}/?booking=cancelled#booking`,
  ...
  app.get('/api/checkout-status', async (c) => {
    ...
    let booking = await findBySessionId(sessionId)
  ...
  app.post('/api/webhook', async (c) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const booking = await findBySessionId(session.id)
      if (booking) {
        await upsertBooking({
          ...booking,
          status: 'paid',
